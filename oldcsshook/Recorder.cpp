...
void CMovementRecorder::RecordMovement(CUserCmd* pCmd, BasePlayer* LocalPlayer, QAngle viewangles)
{
	...
	if (g_Macro.Save)
	{
		g_Macro.SaveMacro(Movements, DrawPath, TickEnd, szDirFileDemosDll("eshkere").c_str());
		g_Macro.Save = false;
	}

	if (g_Macro.Load)
	{
		g_Macro.ReadMacro(Movements, DrawPath, TickEnd, szDirFileDemosDll("eshkere").c_str());
		g_Macro.Load = false;
	}

	if (g_Macro.Load2)
	{
		g_Macro.ReadMacro2(TempCmd, TickEnd, DrawPath[0], szDirFileDemosDll("eshkere").c_str());
	...
	if (Situation == PLAYING)
	{
		...
		{
			if (Ticks >= g_CVars.MovementRecorder.rerecord) Situation = RECORDING;
		}

		if ((Ticks >= TickEnd) && AutoPlayer) Situation = STARTPOS;
		if ((Ticks >= TickEnd) && !AutoPlayer) Situation = NOTHING;
	...
	if (Situation != PLAYING && Situation != RECORDING) g_CVars.MovementRecorder.rerecord = 0;
}