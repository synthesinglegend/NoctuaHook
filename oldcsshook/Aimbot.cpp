...
void Aimbot::Main(CUserCmd* pCmd, BasePlayer* LocalPlayer)
{
	...
	for (auto i = g_pGlobals->maxClients; i >= 1; --i)
	{
		...
		GetAsyncKeyState(g_CVars.Aimbot.mindmgpressedvalue) ? g_CVars.Aimbot.MinDamage : g_CVars.Aimbot.mindmgpressed;
		...
		else
		{
			...
			{
				int hitbox = (g_CVars.Aimbot.HitScan == 0) ? iSpot : Choose[m_iHitbox];
