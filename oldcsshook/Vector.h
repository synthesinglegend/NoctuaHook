...
__forceinline  void VectorTransform( const float *in1,const matrix3x4_t & in2, float *out )
{
...
__forceinline  void AngleVectors( const QAngle &angles, Vector* forward)
{
	float sp, sy, cp, cy;
...
__forceinline  void AngleVectorsTranspose (const QAngle &angles, Vector *forward, Vector *right, Vector *up)
{
	float sr, sp, sy, cr, cp, cy;
...
__forceinline  void VectorAngles( const Vector& forward, QAngle &angles )
{
...
__forceinline  float fastskrrrt( float ehi )
{
	float tmp = ehi;
	float half = 0.5f * ehi;
...
__forceinline  void VectorToEulerAngles( const Vector& forward, QAngle &angles )
{
	float tmp, yaw, pitch;
