...
WeaponInfo NoSpread::GetWeaponInfo(CSWeapon* Weapon)
{
	...
	//Weapon->GetWeaponBulletType( TempWeaponInfo.BulletType );
	...
	//Weapon->GetBulletTypeParameters( TempWeaponInfo.BulletType, TempWeaponInfo.PenetrationPower, TempWeaponInfo.PenetrationRange );
...
void NoSpread::Main(CUserCmd* pCmd, QAngle& In, BasePlayer* LocalPlayer, CSWeapon* Weapon, bool isAA)
{
	...
	else if (g_CVars.Accuracy.NoSpreadMode == 3)
	{
	...
	vecDir = vecForward + ( vecRight * SpreadCone * vecRandom[0] ) + ( vecUp * SpreadCone * vecRandom[1] );
	vecDir.Normalize();
